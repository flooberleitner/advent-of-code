...
class Brute
  ...
  def initialize(name:, salt:, key_count:, stretched_hash:)
    @salt = salt
    @name = name
...
brute = Brute.new(name: 'Part 1', salt: 'cuanljph', key_count: 64, stretched_hash: true)
brute.collect

puts "\nPuzzle 2016/14 results"
puts "  #{brute.name}: #{brute.valid_keys.keys.sort.last} (corr: 23769)"
